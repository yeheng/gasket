...
mod feishu_e2e {
    ...
    async fn test_feishu_get_access_token_real_api() {
        ...
        let result = gasket_core::channels::base::Channel::start(&mut channel).await;
        assert!(
            result.is_ok(),
    ...
    async fn test_feishu_send_text_real_api() {
        ...
        let start_result = gasket_core::channels::base::Channel::start(&mut channel).await;
        assert!(
            start_result.is_ok(),
            "Feishu start failed: {:?}",
            start_result.err()
    ...
    async fn test_feishu_send_via_stateless_api() {
        ...
        gasket_core::channels::base::Channel::start(&mut channel)
            .await
...
mod telegram_e2e {
    ...
    async fn test_telegram_send_message_real_api() {
        load_env();
        use gasket_core::channels::telegram;
...
mod wecom_e2e {
    use super::*;
    use gasket_core::channels::wecom::{WeComChannel, WeComConfig};
